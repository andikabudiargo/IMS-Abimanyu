...
class StockReportExport implements FromArray, WithTitle, WithStyles, ShouldAutoSize
{
    ...
    public function array(): array
    {
        ...
        $out = [
            ...
            [],
            array_merge(['No', 'Alt. Code', 'Article Desc', 'Supp', 'UoM', 'Opening'], array_column($mv, 'label'), ['Balance']),
        ];

        foreach ($this->data['rows'] as $r) {
            $row = [$r['no'], $r['alt_code'], $r['article_desc'], $r['supp'], $r['uom'], $r['opening']];
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        $sheet->getStyle('F6:' . $sheet->getHighestColumn() . $this->lastRow)->getNumberFormat()->setFormatCode('#,##0.00');
        return [];
    }
}
