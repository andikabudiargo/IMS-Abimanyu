    ...
    foreach ($articleCodeRm as $rm) {
        ...
        if ($article) {
            $dataSetRm[] = [
                ...
                'qty' => isset($rm->qty) && is_numeric($rm->qty) && $rm->qty > 0 ? $rm->qty : 1,
                'uom' => $article->uom,
                'urutan' => $urutanRm++,
                'created_at' => now(),
                'updated_at' => now(),
