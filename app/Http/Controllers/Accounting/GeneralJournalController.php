...
class GeneralJournalController extends Controller
{
    ...
    public function show(Request $request)
    {
        ...
        return view("accounting.jurnalUmum.show",$data);
    ...
    public function list(Request $request)
    {
        ...
        ->addColumn('action', function ($data) use ($lockDateToDate) {
            ...
            if ( $data->statusku == '2' or $data->statusku == '1') {
                // if (Auth::user()->can('jurnalUmum-approve')) {
            ...
            // if (Auth::user()->can('bankKeluar-edit')) {
                ...
                // if ( $data->statusku == '2' or $data->statusku == '1') {
                $gjDate = date('Y-m-d', strtotime($data->voucher_date_2));
                if($gjDate>=$lockDateToDate){
                    $buttons .=     '<a href="'. route('jurnalUmum.edit', ['id'=>Crypt::encryptString($data->id)]) .'" class="dropdown-item">
            ...
            $buttons .=         '<a href="'. route('jurnalUmum.print', ['id'=>Crypt::encryptString($data->id)]) .'" target="_blank" class="dropdown-item">
                                    <i data-feather="printer"></i>
                                    Print
                                </a>';
            
            
            // if (Auth::user()->can('jurnalUmum-delete')) {
            if ($data->statusku != '5') {
                $gjDate = date('Y-m-d', strtotime($data->voucher_date_2));
                if($gjDate>=$lockDateToDate){
                    $buttons .=         "<a href='javascript:;'
                                    ...
                                    data-href='". route("jurnalUmum.destroy", ['id'=>Crypt::encryptString($data->id)]) ."'>
    ...
    public function print(Request $request)
    {
        ...
        return view('accounting.jurnalUmum.print',$data);

        // view()->share($data);

        // $pdf = PDF::loadView('accounting.jurnalUmum.print');
