...
class BankReceiptController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view("bankReceipt.index",$data);
    }
    ...
    {   
        ...
        ->select('invoice_number',
                 'invoice_date',
                 DB::raw('(SELECT sum((qty*price)) +sum((qty*price_service)) from invoice_det where invoice_number = invoice_hdr.invoice_number) as basis_amount'),
                 DB::raw('(SELECT sum(ppn) from invoice_det where invoice_number = invoice_hdr.invoice_number) as vat'),
                 DB::raw('(SELECT sum(pph23) from invoice_det where invoice_number = invoice_hdr.invoice_number) as pph23'),
                 DB::raw('(SELECT sum(((qty*price)+(qty*price_service)+ppn)-pph23) from invoice_det where invoice_number = invoice_hdr.invoice_number) as total'),
                 DB::raw("concat(third_party.kode,'-',third_party.nama) as customer"))
        ->where(function ($query) use ($customer,$invFromDate,$invToDate,$dueFromDate,$dueToDate) {
            $customer ? $query->where('customer_id',$customer) : '';
            $invFromDate ? $query->whereBetween(DB::raw("to_date(invoice_date,'DD-MM-YYYY')"), [$invFromDate, $invToDate]) : '';
        ...
        ->addColumn('select_orders', static function ($result) {
            return '<input type="checkbox" class="select-checkbox" name="invCheck[]" value="'.$result->invoice_number.'"/>';
        })
        ->addColumn('paid_on', static function ($result) {
            return '<input type="text" id="row-1-age" name="row-1-age" value="61">';
        })
        
        ->rawColumns(['select_orders','paid_on'])
        ->make(true);
    }
    ...
    {   
        ...
        $dnNumber = substr($dnNumber,0,-1);     
        $dnNumber = explode(",",$dnNumber);
        $customer = $request->customer;
        $invFromDate = "";
        $invToDate = "";
        $dueFromDate = "";
    ...
    public function create(Request $request)
    {
        ...
        return view("bankReceipt.create",$data);
    }
        ...
        try {
                ...
                foreach ($details as $val) {
                    if ($val->type =="ap" ){
                        ...
                        ->leftJoin('third_party','third_party.kode','ap_invoice.customer_id')
                    ...
                    if ($val->type =="pi" ){
                        ...
                        ->leftJoin('third_party','third_party.kode','ap_pro_invoice.customer_id')
                        ->select(DB::raw("'$disbursementNumber' as disbursement_number"),
                                'pi_number',
                                DB::raw("'$val->type' as ref_type"),
                                'inv_number',
    ...
    public function update(Request $request)
    {
        ...
        $suppCode = $request->input('customer');
