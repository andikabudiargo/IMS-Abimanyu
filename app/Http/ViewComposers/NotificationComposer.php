...
class NotificationComposer
{
    public function compose(View $view)
    {
        ...
        $lists['listPo2'] = DB::select("SELECT * from (
            select id,po_number,po_date,'$username' as username,
            coalesce((select max(approval_order) from approval_history where module_code = 'PO' and module_number = po_number),0) as sudah_approve,
            coalesce((select approval_order from approval_level where username = '$username' and module_code = 'PO'),0) as berhak_approve,
            (SELECT sum(qty*price) from purchase_order_det where po_number = purchase_order_hdr.po_number) as po_amount,
