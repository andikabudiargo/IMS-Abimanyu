...
Route::get('/tmp-recalc-sto-phantom', function () {
    ...
    foreach ($configs as $configId) {
        $ctrl->recalcTargetAct($configId);
        echo "Config $configId done<br>";
    }
...
Route::group( ['middleware' => ['auth']], function() {
	...
	Route::post('articles/safetyStock/import',['as'=>'articles.safetyStock.import.excel','uses'=>'ArticleController@safetyStockImportExcel']);
...
Route::prefix('supplier-replace')->group(function () {
	...
	Route::get('apPaymentSchedule', ['uses' => 'ApPaymentScheduleController@index','as'   => 'apPaymentSchedule.index',]);
	Route::post('apPaymentSchedule/data', ['uses' => 'ApPaymentScheduleController@data','as'   => 'apPaymentSchedule.data',]);
	Route::post('apPaymentSchedule/detail', ['uses' => 'ApPaymentScheduleController@detail','as'   => 'apPaymentSchedule.detail',]);
	Route::post('apPaymentSchedule/export', ['uses' => 'ApPaymentScheduleController@export','as'   => 'apPaymentSchedule.export',]);
	...
	Route::post('production/actual-loading/cancel',['as' =>'production.actualLoading.cancel', 'uses'=>'Production\ActualLoadingController@cancel']);
	...
	Route::get('actual-finish-goods/search-article-fg', ['as' => 'production.actualFinishGoods.searchArticleFg', 'uses' => 'Production\ActualFinishGoodsController@searchArticleFg']);
...
Route::post('location/delete',       'LocationController@destroy')->name('location.destroy');
	...
	Route::get('transferIn/lastStatus',['as'=>'transferIn.lastStatus','uses'=>'Transfer\TransferInController@lastStatus']);	
...
Route::post('stock-adjustment/stock-before-bulk', ['as' => 'stockAdjustment.stockBeforeBulk', 'uses' => 'StockAdjustmentController@stockBeforeBulk']);
...
Route::post('stock-taking-order/{id}/recalc-accuracy', [
    'as' => 'stockTakingOrder.recalcAccuracy',
    'uses' => 'StockCountController@requestRecalcAccuracy'
]);

Route::get('stock-taking-order/recalc-status/{jobToken}', [
    'as' => 'stockTakingOrder.recalcStatus',
    'uses' => 'StockCountController@checkRecalcAccuracyStatus'
]);
...
Route::prefix('stockConsumption')->name('stockConsumption.')->group(function () {
	...
	Route::get('conversion/price-list',           'Conversion\PriceListController@index')->name('conversion.priceList.index');
Route::post('conversion/price-list/get-bom',  'Conversion\PriceListController@getBom')->name('conversion.priceList.getBom');
Route::get('conversion/price-list/create', 'Conversion\PriceListController@create')->name('conversion.priceList.create');
...
Route::post('conversion/price-list/destroy', 'Conversion\PriceListController@destroy')->name('conversion.priceList.destroy');
Route::get('conversion/price-list/list', 'Conversion\PriceListController@list')->name('conversion.priceList.list');
...
Route::post('conversion/price-list/import-excel', 'Conversion\PriceListController@importExcel')->name('conversion.priceList.import.excel');
	...
	Route::get('conversionReport/chart',['as'=>'conversionReport.chart','uses'=>'Conversion\ConversionReportController@chart']);
	...
	Route::post('conversionReport/update',['as'=>'conversionReport.update','uses'=>'Conversion\ConversionReportController@update']);
	Route::post('conversionReport/revision',['as'=>'conversionReport.revision','uses'=>'Conversion\ConversionReportController@revision']);
	Route::post('conversionReport/cancel',['as'=>'conversionReport.cancel','uses'=>'Conversion\ConversionReportController@cancel']);
	...
	Route::get('/test-memory', function() {
		// Set memory limit
		$memoryBefore = ini_get('memory_limit');
		$memoryResult = ini_set('memory_limit', '512M');
		$memoryAfter = ini_get('memory_limit');
		
		// Set time limit
		$maxExecutionTimeBefore = ini_get('max_execution_time');
		set_time_limit(300); // 5 minutes
		$maxExecutionTimeAfter = ini_get('max_execution_time');
		
		$info = [
			// Memory Information
			'memory_limit_before' => $memoryBefore,
			'memory_limit_after' => $memoryAfter,
			'memory_set_success' => ($memoryAfter === '512M') ? 'Yes' : 'No',
			'memory_ini_set_result' => $memoryResult,
			
			// Time Limit Information
			'max_execution_time_before' => $maxExecutionTimeBefore,
			'max_execution_time_after' => $maxExecutionTimeAfter,
			'time_set_success' => ($maxExecutionTimeAfter == 300) ? 'Yes' : 'No',
			
			// Current Usage
			'current_memory_usage' => round(memory_get_usage() / 1024 / 1024, 2) . ' MB',
			'peak_memory_usage' => round(memory_get_peak_usage() / 1024 / 1024, 2) . ' MB',
			'script_execution_time' => round(microtime(true) - LARAVEL_START, 2) . ' seconds'
		];
