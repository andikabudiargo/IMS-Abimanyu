                        ...
                        <form id="frmAdd" name="frmAdd" autocomplete="off">
                            ...
                            <div class="form-row">
                                <div class="form-group col-md-6">
                                    ...
                                    <select class="select2 form-control" id="supplierCode" name="supplierCode">
...
@include('forecasting.purchasing.addArticle')
@endsection
@section('styles')
...
<script type="text/javascript">
    ...
    $('#supplierCode,#year,#bulanAwal,#bulanAkhir').change(function(e){
        let $this= $(this);
        ...
        if ($('#supplierCode').val()){            
            ...
            $.ajax({
                url:"{{route('forecastPurchase.get.list.article')}}",
                method:"POST",
                data:{
                ...
                success:function(result){
                    ...
                    for(i=0;i< result.data.length;i++){
                        list=`<td ><button class="btn btn-danger btn-sm" type="button" onclick="deleteArticle('${result.data[i].supplier_id}','${result.data[i].article_code}','${result.data[i].year}','${result.data[i].article_desc}')" id="cmdEdit" name="cmdEdit" >Delete</button> 
    ...
    $('body').on('change', '#articleId', function() {
        ...
        if ($this.val()){
            ...
            $.ajax({
                url:"{{route('forecastPurchase.get.qty.article')}}",
                method:"POST",
                data:{
                    supplierCode:supplier,
                    article:article,
    ...
    deleteArticle = (supplierId,articleCode,year,articleDesc) =>{
        Swal.fire({
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url:"{{route('forecastPurchase.destroy')}}",
                    method:"POST",
                    data:{
                    ...
                    success:function(result){
                        $('#supplierCode').val(supplierId).trigger('change');
    ...
    $("#cmdSave").click(function(){
        ...
        $(".data-bulan").map(function(i) {  
            ...
            details.push({
                "fc_code": supplierCode+year+month,
        ...
        if (flag == 0){
            let supplierCodeIni= $('#supplierCode').val();
            $.ajax({
                type: "post",
                url: "{{ route('forecastPurchase.store') }}",
                data: {
                ...
                success: function(data) {
                    if (data.status == 0 ){
                        ...
                        emptyList();
                        $('#supplierCode').val(supplierCodeIni).trigger('change');
