...
#scheduleTable .col-supplier { position: sticky; left: 0; min-width: 220px; max-width: 220px;
    white-space: normal; background: inherit; z-index: 2; }
#scheduleTable thead th.col-supplier { z-index: 5; background: #e4e9f1 !important; }
...
#scheduleTable .col-supplier { left: 40px; }
#scheduleTable thead th.col-supplier { left: 40px; }
...
<section id="schedule-filter">
    <div class="card">
        <div class="card-header">
            <h4 class="card-title">Filter AP Payment Schedule</h4>
                ...
                <div class="form-row">
                    <div class="col-12">
                        <small class="text-muted d-block mb-1">
                            <strong>Opening</strong> = saldo AP yang jatuh tempo sebelum awal bulan terpilih.
                            Kolom tanggal = AP yang jatuh tempo pada tanggal itu di bulan terpilih.
            ...
            <div class="card border-0 shadow-none">
                <div class="card-body d-flex align-items-center p-1">
                    ...
                    <div>
                        ...
                        <small class="text-muted">Total Jadwal Bayar</small>
                    </div>
                </div>
            </div>
        </div>
...
<section id="schedule-result">
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center flex-wrap">
            <h4 class="card-title mb-0">Hasil AP Payment Schedule</h4>
    ...
    <div class="modal-dialog modal-lg modal-dialog-scrollable" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="scheduleDetailTitle">Detail AP</h5>
...
$(document).ready(function () {
    ...
    function generateReport() {
        ...
        $.post("{{ route('apPaymentSchedule.data') }}", lastFilters)
        .done(function (res) {
    ...
    function buildFrame(daysInMonth, year, month) {
        ...
        for (let d = 1; d <= daysInMonth; d++) {
            ...
            h += '<th class="schedule-clickable' + cls + '" data-bucket="d' + d + '" data-supplier="">'
                + '<span class="dnum">' + d + '</span><span class="dow">' + DOW[g] + '</span></th>';
        }
        h += '<th class="schedule-clickable" data-bucket="total" data-supplier="">Total</th>'
            + '<th>Paid</th>'
            + '<th>Balance</th>'
            + '<th class="schedule-clickable" data-bucket="outstanding" data-supplier="">Outstanding</th>'
            + '</tr>';
        ...
        f += '<td class="schedule-clickable" id="tTotal" data-bucket="total" data-supplier="">0</td>'
            + '<td id="tPaid">0</td>'
            + '<td id="tBalance">0</td>'
            + '<td class="schedule-clickable" id="tOutstanding" data-bucket="outstanding" data-supplier="">0</td>'
            + '</tr>';
        ...
        if (!res.rows || res.rows.length === 0) {
            ...
            res.rows.forEach(function (r, idx) {
                ...
                function cell(bucket, val, cls, remain) {
                    ...
                    return '<td class="' + c + ' schedule-clickable" data-supplier="' + sc + '" data-bucket="' + bucket + '">' + fmt(val) + '</td>';
                }
    ...
    $('#scheduleTable').on('click', 'td.schedule-clickable, th.schedule-clickable', function () {
        ...
        $('#scheduleDetailTitle').text('Detail AP - ' + bucketTitle(bucket) + (supplier ? ' - ' + $(this).closest('tr').find('.col-supplier').text() : ' (Semua Supplier)'));
        ...
        $.post("{{ route('apPaymentSchedule.detail') }}", {
    ...
    $('#btnPrint').on('click', function () {
        ...
        w.document.write('<html><head><title>AP Payment Schedule</title>');
        w.document.write('<style>'
            ...
            + '</style></head><body>');
        w.document.write('<h3 style="margin-bottom:4px;">AP Payment Schedule</h3>');
    ...
    $('#btnExport').on('click', function () {
        let $f = $('<form>', { method: 'POST', action: "{{ route('apPaymentSchedule.export') }}" });
