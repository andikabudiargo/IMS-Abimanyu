...
<body>
    <div class="hide-print" style="margin:20px">
    ...
    @foreach ($halaman as $h => $baris)
        ...
        <div class="sheet">
            ...
            {{-- ===== BLOK BAWAH ===== --}}
            @if($isLast)
                {{-- Totals + Note + tanda tangan: hanya halaman terakhir --}}
                <div class="blok-bawah">
                    <table id="tblTotal">
                        @foreach ($totals as $val)
                            <tr>
                                <td colspan="3" rowspan="5" style="border-bottom:1px solid #000;vertical-align:top;">
                                    <b>Terbilang : </b>
                                    <i class="arial" style="font-size:10pt;">{{ ucwords(strtolower($terbilang)) }}</i>
                                </td>
                                <td width="10.6%">Selling Price</td>
                                <td width="13.9%" align="right">{{ number_format($val->sub_total,2) }}</td>
                            </tr>
                            <tr><td>VAT Object</td><td align="right">{{ number_format($val->dpp_lain_value,2) }}</td></tr>
                            <tr><td>VAT {{ $nilaiPPN }}%</td><td align="right">{{ number_format($val->ppn,2) }}</td></tr>
                            <tr><td>WHT 23</td><td align="right">{{ $val->pph23 ? '-'.number_format($val->pph23,2) : '-' }}</td></tr>
                            <tr><td>Total Bill</td><td align="right">{{ number_format($val->grand_total,2) }}</td></tr>
                        @endforeach
                        <tr>
                            <td class="arial catatan" valign="top" width="60%" colspan="3" style="border:none;">
                                Note:<br>
                            ...
                            <td class="arial catatan" valign="top" colspan="2" align="center" style="border:none;">
                    ...
                    <table style="width:100%;margin-top:2px;">
                        <tr>
                            <td class="arial footnote">
                                <i>Lembar Asli untuk Penagihan kepada Customer</i><br>
                                <i>Lembar Copy untuk Arsip</i>
                            </td>
                            <td class="footnote" align="right" valign="bottom" width="15%">
                                Page {{ $h+1 }} of {{ $jmlHalaman }}
                            </td>
                        </tr>
                    </table>
                </div>
            @else
                ...
                <div class="blok-bawah">
                    <table style="width:100%;">
                        <tr>
                            <td></td>
                            <td class="footnote" align="right" width="15%">Page {{ $h+1 }} of {{ $jmlHalaman }}</td>
                        </tr>
                    </table>
                </div>
            @endif
