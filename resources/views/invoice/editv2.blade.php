...
<section id="edit-index">
                                        ...
                                        <div class="form-group col-md-12">
                                            ...
                                            <select class="select2 form-control" id="customer" name="customer" required disabled>
                                                ...
                                                @foreach($customers as $val)
                                                    <option value="{{$val->kode}}" data-coa = "{{ $val->account }}" data-top="{{ $val->top_batas_1 }}" {{$val->kode == $header->customer_id ? "selected" : ""}} >{{$val->kode}} - {{$val->nama}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
    ...
    <div class="form-group col-md-4">
        ...
        <input type="date" id="jatuhTempo" name="jatuhTempo" class="form-control" value="{{ $header->jatuh_tempo }}">
    </div>
</div>
...
function hitungJatuhTempo() {
    ...
    let d = new Date(parseInt(arr[2]), parseInt(arr[1]) - 1, parseInt(arr[0]));
...
$('#customer').on('change', function() {
    ...
    $("#cmdSave").click(function(){
        if (!$("#frmAdd")[0].checkValidity()){
            ...
            // getBuktiPotong($('#buktiPotong').val(),$('#customer').val(),$('#invNumber').val(), function(error, result) {
            //     if (error) {
