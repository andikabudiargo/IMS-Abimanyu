...
<section id="add-index">
    <div class="row">
        <div class="col-8">
            <div class="card">
                <div class="card-body">
                    <form id="frmAdd" name="frmAdd" action="{{ route('location.update', ['id' => $encId]) }}" method="post" autocomplete="off">
                        @csrf
                        <div class="row">
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label for="location_code">Location Code</label>
                                    <input type="text" id="location_code" class="form-control text-uppercase"
                                           value="{{ $loc->location_code }}" disabled />
                            ...
                            <div class="col-md-9">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="location_name" name="location_name" class="form-control text-uppercase"
                                           value="{{ old('location_name', $loc->location_name) }}" required maxlength="150" autofocus />
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <select class="select2 form-control" id="location_type" name="location_type">
                                    ...
                                    @foreach($locationTypes as $val => $label)
                                        <option value="{{ $val }}" {{ old('location_type', $loc->location_type) === $val ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group col-md-6">
                                ...
                                <select class="select2 form-control" id="dept_code" name="dept_code">
                                    <option value=""></option>
                                    @foreach($depts as $d)
                                        <option value="{{ $d->code }}" {{ old('dept_code', $loc->dept_code) === $d->code ? 'selected' : '' }}>{{ $d->code }} — {{ $d->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="form-group col-md-12">
                                ...
                                <select class="select2 form-control" id="article_type" name="article_type[]" multiple>
                                    @foreach($articleTypes as $val => $label)
                                        <option value="{{ $val }}" {{ in_array($val, (array) old('article_type', $locArticleType)) ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        @php $locationKind = old('location_kind', $loc->parent_location ? 'child' : 'parent'); @endphp
                        <div class="row">
                            <div class="form-group col-md-12">
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <select class="select2 form-control" id="parent_location" name="parent_location">
                                    ...
                                    @foreach($parents as $p)
                                        <option value="{{ $p->location_code }}" {{ old('parent_location', $loc->parent_location) === $p->location_code ? 'selected' : '' }}>{{ $p->location_code }} — {{ $p->location_name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row">
                            <div class="form-group col-md-4">
                                ...
                                <select class="select2 form-control" id="status" name="status">
                                    <option value="1" {{ old('status', $loc->status ?? '1') == '1' ? 'selected' : '' }}>Aktif</option>
                                    <option value="0" {{ old('status', $loc->status ?? '1') == '0' ? 'selected' : '' }}>Non-Aktif</option>
                                </select>
                            </div>
                            <div class="form-group col-md-8">
                                <label for="pic">PIC</label>
                                <input type="text" id="pic" name="pic" class="form-control" value="{{ old('pic', $loc->pic) }}" maxlength="100" placeholder="opsional" />
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="form-group">
                                    ...
                                    <textarea id="note" name="note" class="form-control" rows="2" maxlength="500" placeholder="opsional">{{ old('note', $loc->note) }}</textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-12">
                                <a href="{{ route('location.index') }}" class="btn btn-outline-secondary">Cancel</a>
