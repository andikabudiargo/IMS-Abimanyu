...
<section id="add-index">
    <div class="bs-stepper wizard-modern tab-delivery-plan">
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class="table-scrollable" style="height: 500px;">
                            ...
                            <table cellspacing="0" id="tblBaru" class="table table-sm table-bordered" style="min-width: 1200px;">
                                <thead>
                                </thead>
                                <tbody>
                                </tbody>
...
<script type="text/javascript">
    ...
    if (typeof tabDeliveryPlan !== undefined && tabDeliveryPlan !== null) {
        var modernStepper = new Stepper(tabDeliveryPlan, {
        linear: false
        });
        $(tabDeliveryPlan)
        .find('.btn-next')
    ...
    $("#cmdSubmitFilter").click(function(e){
        ...
        if (soDate){
            $(function(){
                var oTable =$("#tblSoList").DataTable({
                    ajax:{
    ...
    function detaiArticle(soNumber) {
        let isidata = $('#tblArtList tr').length;
        if (isidata >0){
            let table= $('#tblArtList').DataTable();
            table.destroy();
            $('#tblArtList tbody > tr').remove();
            let celli="";
            $("#tblArtList thead > tr").remove();
        }
        $(function(){
            let aTable =$("#tblArtList").DataTable({
                ajax: {
                ...
                columns: [
                    { data: 'select_orders', name: 'select_orders',title: 'Check', searchable: false, orderable: false },
                    { data: 'so_code', name: 'so_code',title:'SO Code' },
                    { data: 'article_alternative_code', name: 'article_alternative_code',title:'Article' },
