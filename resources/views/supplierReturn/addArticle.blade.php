...
<script type="text/javascript">
    ...
    function loadArticleBySupplierLocation(){
        let supplierCode = objSupplier.val();
        let locationNumber = objLocation.val();
        if (!locationNumber){
            dataArticle = '<option value="">Choose article</option>';
            return;
        }
        $.ajax({
            url:"{{ route('supplierReturn.get.article') }}",
            method:"POST",
            data:{
    ...
    $('#supplier').on('change', function() {
        loadArticleBySupplierLocation();
    });

    $('#location').on('change', function() {
        loadArticleBySupplierLocation();
        // reset baris yang sudah ada karena stok tergantung lokasi
        $('#article_row').empty();
        cloneCount = 0;
        recordCount();
    });
    ...
    function disabledEnabledSelect2(){
        ...
        if (records > 0){
            ...
            objLocation.attr('disabled','disabled');
        }else{
            objSupplier.removeAttr('disabled');
            objLocation.removeAttr('disabled');
        }
    }
