                        ...
                        <ul class="nav nav-tabs" role="tablist">
                            @foreach( $headers as $key =>$header )
                                <li class="nav-item">
                                    ...
                                    data-po-number="{{ $header->po_number }}">{{ $key == 0 ? 'Main':'Revision '.($key-1) }}</a>
                                </li>
                            @endforeach
                        </ul>
                                            ...
                                            @foreach( $details as $key =>$item )
                                                @if($item->po_number === $header2->po_number )
                                                    <tr>
                                                        ...
                                                        <td class="text-right">{{ $item->uom_group =='PIECE' ? number_format($item->qty_stock) : number_format($item->qty_stock,2) }} {{ $item->article_uom }}</td>
                                                        <td class="text-right">{{ $item->uom_group =='PIECE' ? number_format($item->qty) : number_format($item->qty,2) }} {{ $item->article_uom }}</td>
